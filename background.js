  ...
  return new Promise((resolve) => {
    chrome.storage.local.get([STORAGE_KEY], (result) => {
      try {
        ...
        const currentGroup = parsed.currentGroup && groups[parsed.currentGroup] ? parsed.currentGroup : 'Default';
...
function isDuplicateEntry(entry, groupName, state) {
  ...
  return items.some((item) => {
    const samePair = (item.key || '').trim().toLowerCase() === (entry.key || '').trim().toLowerCase() && (item.value || '').trim().toLowerCase() === (entry.value || '').trim().toLowerCase();
    const sameValue = (item.value || '').trim().toLowerCase() === (entry.value || '').trim().toLowerCase();
